...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач ДополнительныеПараметры) Экспорт
	...
	АдресСервера = ПараметрыКоманды.Сервер;
	Токен = ПараметрыКоманды.Токен;
	Проект = ПараметрыКоманды.ИДПроекта;

	ГитЛаб = Новый GitLabIntegrator(АдресСервера, Токен);
	ГитЛаб.ПолучитьПоследнийРелиз("", Проект);
