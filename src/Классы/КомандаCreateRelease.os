...
Функция ПолучитьКаталогАктуальнойВерсии(Знач Путь)
	...
	АктуальнаяВерсия = "v0.0";
	Для каждого Каталог Из Каталоги Цикл

		ТекущаяВерсия = Каталог.Имя;	
		МассивТекущаяВерсия  = СтрРазделить(СтрЗаменить(ТекущаяВерсия, "v", ""), ".");
		МассивАктуальнаяВерсия  = СтрРазделить(СтрЗаменить(АктуальнаяВерсия, "v", ""), ".");
		...
		Для Индекс = 0 По МассивТекущаяВерсия.ВГраница() Цикл
			Если Число(МассивТекущаяВерсия[Индекс]) > Число(МассивАктуальнаяВерсия[Индекс]) Тогда
					Лог.Отладка("Актуальная версия: " + МассивАктуальнаяВерсия[Индекс]);
					Лог.Отладка("Текущая версия: " + МассивТекущаяВерсия[Индекс]);
		...
		Лог.Отладка("Текущая версия: " + ТекущаяВерсия);
